...
class ChainStore {
	...
	clearCache() {
		...
		this.get_account_refs_of_keys_calls = {};
	...
	/**
	 *  @param key the public key to find accounts that reference it
	 *
	...
	getAccountRefsOfKey(key) {
		if(this.account_ids_by_key.has(key)) {
			return this.account_ids_by_key.get(key);
		}
		
		return new Promise((resolve, reject) => {
			if(typeof this.get_account_refs_of_keys_calls[key] != 'undefined') {
				return this.get_account_refs_of_keys_calls[key].push([resolve, reject]);
			}
			this.get_account_refs_of_keys_calls[key] = [[resolve, reject]];
			...
			Apis.instance().db_api().exec("get_key_references", [[key]]).then(vec_account_id => {
					...
					this.get_account_refs_of_keys_calls[key].forEach(row => row[0]({refs, key}));
					delete this.get_account_refs_of_keys_calls[key];
				},
				error => {
					this.account_ids_by_key = this.account_ids_by_key.delete(key);
					this.get_account_refs_of_keys_calls[key].forEach(row => row[1]());
					delete this.get_account_refs_of_keys_calls[key];
				});
